...
eval(
  "bun -e \"import('./src/db.js').then(async ({DB}) => { const os=await import('os'); const fs=await import('fs'); const path=await import('path'); const dir=fs.mkdtempSync(path.join(os.tmpdir(),'iodb-eval-')); const db=DB('io',{path:dir}); const kv=db.kv(); kv.in({hello:'world'}); console.log(kv.get('hello'))})\"",
