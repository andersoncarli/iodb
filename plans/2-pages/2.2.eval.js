// Roteiro de avaliacao — feature 2.2: a secao critica ficou minima.
...
//    dentro do io-engine.js. E o que permite um segundo consumidor.
eval("test -f io-append.js && echo OK", (out) => check(out.includes("OK")))
eval("grep -c '^export function' io-append.js", (out) => check(Number(out.trim()) >= 4))
...
eval("grep -A2 'export function ensureLock' io-append.js", (out) => {
  check(out.includes("'wx'"))
  check(!out.includes("existsSync"))
})
...
eval("sed -n '340,355p' io-engine.js | grep -n 'release\\|saveIndex()\\|publishYaml()'", (out) => {
...
eval("grep -c 'indexOffsetOnDisk' io-engine.js", (out) => check(Number(out.trim()) >= 2))
eval("grep -A3 'const onDisk = indexOffsetOnDisk()' io-engine.js", (out) =>
  check(out.includes("return"))
)

// 6. O RESULTADO — a secao critica parou de crescer com o tamanho do store.
//    Este e o criterio de aceitacao da feature. Baseline 2.1: p99 17ms (1k) e
//    96ms (10k), subindo a 733ms em 100k.
eval("bun io-engine.bench.js --quick", (out) => {
...
eval("grep -c 'CELL_BUDGET_MS' io-engine.bench.js", (out) => check(Number(out.trim()) >= 3))
eval("grep -c 'Date.now() < deadline' io-engine.bench.js", (out) => check(Number(out.trim()) >= 1))

// 8. O resultado medido esta commitado ao lado do baseline que ele derruba.
eval("test -f bench/resultado-2.2.txt && echo OK", (out) => check(out.includes("OK")))
eval("grep -ci plana bench/resultado-2.2.txt", (out) => check(Number(out.trim()) >= 1))

// 9. SUITE VERDE — verificada pelo `sprint test 2.2` (verify_tests), que e quem
...
//
//    Aqui roda so o arquivo que o corte da 2.2 poderia ter quebrado, isolado.
eval("bun ../utest/utest.js io-engine.test.js --force", (out) => {
