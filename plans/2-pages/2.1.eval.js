// Roteiro de avaliação — feature 2.1: benchmark de seção crítica, tornar
// "sub-milissegundo" uma afirmação verificável.

// 1. io-engine.bench.js existe e é reconhecido pelo runner (TEST.yaml inclui
//    **/*.bench.js) — sem isso o comando de verify_tests nunca acha o arquivo.
eval("test -f io-engine.bench.js && echo OK", (out) => check(out.includes("OK")))
...
eval("bun ../utest/utest.js io-engine.bench.js --force", (out, r) => {
  check(r.exitCode, 0)
...
eval("grep -n 'const t = bench ? { precomputeStart' io-engine.js", (out) =>
  check(out.length > 0)
)
...
eval("test -f bench/baseline-2.1.txt && echo OK", (out) => check(out.includes("OK")))
eval("grep -c '### store=' bench/baseline-2.1.txt", (out) => check(out.trim(), "6"))
eval("grep -c '| critical' bench/baseline-2.1.txt", (out) => check(out.trim(), "6"))
...
eval("grep -A10 'store=1000 procs=1 ' bench/baseline-2.1.txt | grep critical", (out) =>
  check(out.includes("17.00"))
)
eval("grep -A10 'store=100000 procs=1 ' bench/baseline-2.1.txt | grep critical", (out) =>
...
// 2.1 não deve reprovar por causa de um teste alheio conhecidamente flaky;
// roda de novo e falha só se o padrão persistir nas outras 19 checagens.
eval("bun ../utest/utest.js io-engine.test.js io-engine.concurrency.test.js io-engine.bench.js --force", (out, r) =>
  check(r.exitCode, 0)
)
