...
Plano do sprint 007 (feature 4.2).

## Objetivo

Implementar a 4.2 como MODULO, nao so como movimentacao de codigo: a secao critica
...
1. `sprint files --drift` antes de codar — a 4.2 mapeava so `io-engine.js` e o sprint
...
nao atinge `valid=true` e o passo 5 nao comecou. A feature 4.2 continua ⚫.

## Criterio que continua valendo para fechar a 4.2

- p95 do tempo-com-lock em 100k proximo do de 1k (secao critica O(1) no tamanho do
  store), medido contra o baseline de 4.1: hoje 17ms (1k) -> 733ms (100k), com 7/8
